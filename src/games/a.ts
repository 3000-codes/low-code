...
class SnakeGame {
  ...
  constructor (canvas: HTMLCanvasElement, { setScore = () => {}, setMaxScore = () => {} }) {
    // init
  ...
  // 绘制 圆
  circle (ctx: CanvasRenderingContext2D, data: iCircle, sStyle: string, fStyle: string): void {
    ctx.save()
    ctx.beginPath()
    ctx.arc(data.rx, data.ry, data.r, data.start, data.end, data.dir)
    ctx.fillStyle = fStyle || '#fff'
    ctx.strokeStyle = sStyle || '#666'
    ctx.fill()
    ctx.stroke()
    ctx.closePath()
    ctx.restore()
  ...
  // 操作
  startToMove (dir: Direction): void {
  ...
  // 主函数
  changeToDir (dir: Direction): void {
    // 页面初始化
    this.ctx.clearRect(0, 0, this.gridWidth, this.gridWidth)
    this.coord()
    this.drawApple(this.ax, this.ay)
